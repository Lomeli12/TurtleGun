...
public class EntityHandler {
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        if (event.entityLiving != null && !event.entityLiving.worldObj.isRemote && (event.entityLiving instanceof EntityTurtle || event.entityLiving instanceof EntityAggressiveTurtle)) {
            if (event.entityLiving.hasCustomName() && event.entityLiving.getCustomNameTag().equals("Maes Hughes") && EntityUtil.damageFromPlayer(event.source)) {
                ...
                if (entity != null && entity instanceof EntityPlayer) {
                    ...
                    if (player != null) {
                        player.addChatComponentMessage(new ChatComponentTranslation("damage.turtle.fma"));
